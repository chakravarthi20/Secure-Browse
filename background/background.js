    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.action === 'checkHibpBreaches' && message.query) {
            ...
            console.log("Message action not recognized or domain missing:", message);
        }
    });

    function checkDomainBreaches(domain) {
    const apiUrl = `https://haveibeenpwned.com/api/v3/breaches?domain=${encodeURIComponent(domain)}`;
    fetch(apiUrl, {
        method: 'GET',
        headers: {
            'hibp-api-key': hibpApiKey,
    ...
    .then(data => {
        if (data && data.length > 0) {
            console.log(data);
            notifyUserOfDomainBreaches(domain, data);
        } else {
            console.log(`No breaches found for domain ${domain}`);
        }
    })
    .catch(error => console.error('Error fetching domain breaches:', error));
}

function notifyUserOfDomainBreaches(domain, breachData) {
    const message = `Breach detected for ${domain}:\n\n${breachData.map(breach => `Name: ${breach.Name}\nDate: ${breach.BreachDate}\nDescription: ${breach.Description}`).join('\n\n')}`;
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs.length > 0) {
            const activeTab = tabs[0];
            chrome.tabs.sendMessage(activeTab.id, {
                action: 'showBreachAlert',
                domain: domain,
            ...
            console.warn("No active tab found to send the domain breach notification.");
        }
    });
}
